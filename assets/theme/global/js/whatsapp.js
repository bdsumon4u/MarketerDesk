...
//Cloud API 
    ...
    $('#whatsapp_template_id').change(function () {
        ...
        $.each(selectedTemplate.template_information, function(index, element) {
            ...
            var $tabButton = $('<button>')
                ...
                .html('<i class="las la-user"></i> ' + tabLabel);
    ...
    function createInputGroup(type, data) {
        ...
        let input;
        const inputGroup           = $('<div>').addClass('form-item mt-3');
        ...
        if (type === 'Body' || type === 'Footer' || type === 'Header' || type === 'Buttons') {
            ...
            if (data.format === 'IMAGE' || data.format === 'DOCUMENT' || data.format === "VIDEO") {
                ...
                const mediaUploadButton = $('<div>').addClass('i-btn light--btn btn--sm ms-3').html('Add Media<i class="fa-solid fa-paperclip"></i>');
                ...
                mediaUploadInput.on('change', function() {
                    ...
                    if (file) {
                        ...
                        mediaUploadButton.html('Add Media<i class="fa-solid fa-paperclip"></i>');
                    }
                });
    ...
    function whatsAppPreview(data, type, value, previewElement, previewText) {
        ...
        if (previewText && placeholder) {
            ...
            console.error(`Element with class "message-${type.toLowerCase()}" not found or placeholder not found.`);
        }
...
//Node Module Or Without CLoud API
    ...
    $("#media_upload").change(function () {
        ...
        var fileDetailsHTML = '<div class="file__wrapper">' +
            ...
            "<p title='"+ file.type +"'>" + "File Type: " + file.type + '</p>' +
            "<p title='"+ bytesToSize(file.size) +"'>" + "File Size: " + bytesToSize(file.size) + '</p>' +
            '</div>' +
            '</div>' +
            '</div>';
