    ...
    public function store(CampaignRequest $request)
    {
        ...
        if ($request->input('channel') == Campaign::SMS) {
            ...
            if (auth()->user()->sms_gateway == 1) {
                ...
                $defaultGateway = $allowed_access->type == PricingPlan::USER ? Gateway::whereNotNull('sms_gateways')->where("user_id", auth()->user()->id)->where('is_default', 1)->first() :
            ...
            $defaultGateway = $allowed_access->type == PricingPlan::USER ? Gateway::whereNotNull('mail_gateways')->where("user_id", auth()->user()->id)->where('is_default', 1)->first()
            ...
            if ($request->input("whatsapp_sending_mode") == "without_cloud_api") {
                ...
                $defaultGateway = $request->input("whatsapp_device_id") == "-1" ? WhatsappDevice::where('user_id', auth()->user()->id)->where('status', 'connected')->pluck("credentials", "id")->toArray()
                    : WhatsappDevice::where('user_id', auth()->user()->id)->where("id", $request->input("whatsapp_device_id"))->where('status', 'connected')->pluck("credentials", "id")->toArray();
            } else {
            }
        }
