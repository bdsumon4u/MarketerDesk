                ...
                <form action="{{route('admin.whatsapp.store')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-4">
                        <div class="order-2 col-xl-9 order-xl-1">
                            ...
                            <div>
                                <button type="submit" class="i-btn primary--btn btn--lg whatsapp-submit">{{translate("Submit")}}</button>
                            </div>
                        </div>
        ...
        $(`textarea[name=message]`).on('keyup', function(event) {
            ...
            if (character.length > 0) {
                ...
                $(".message--word-count").html(`
                    <span class="text--success character">${character.length}</span> Character |
                    <span class="text--success word">${word.length}</span> Words|
                    <span class="text--success word">${sms}</span> SMS (${wordLength} Char./SMS)`);
            } else {
